...
#     # File uploader for resumes (zip file)
#     uploaded_file = st.file_uploader(
#         "Upload a zip file containing resume files",
#         type=['zip'],
#         key="resume_uploader"
...
#     # File uploader for optional job description (text file)
#     job_description_file = st.file_uploader(
#         "Upload a job description text file (optional)",
#         type=['txt'],
#         key="jd_uploader"
...
# # Display uploaded file names (Maintains space after upload)
# if uploaded_file:
...
#             for resume_id, pdf_path in extracted_pdfs.items():
...
with st.container():
    ...
    uploaded_file = st.file_uploader(
        "Upload a zip file containing resume files", type=["zip"], key="resume_uploader"
    )

    job_description_file = st.file_uploader(
        "Upload a job description text file (optional)", type=["txt"], key="jd_uploader"
    )
    ...
    if uploaded_file is not None:
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            if results is not None and not results.empty:
                ...
                sorted_pdfs = sorted(
                    extracted_pdfs.items(), key=lambda x: top_3_ids.index(x[0])
                )
